...
try:
    ...
    print "NO format_exc in your version of Python. Very strange! Are you running OS X? Email support@immunityinc.com about this"
    format_exc = str
...
def __MOSDEFimport__(*args):
    ...
    try:
        ...
        name=format_exc(0).split("\n")
        if len(name)>0:
            name=name[1]
        else:
            name=name[0]
        devlog('ImportError', name, nodesc = True)
...
def int2str64(int64):
    return int2str_bits(64, int64, swap=0)

def int2str64_swapped(int64):
    return int2str_bits(64, int64, swap=1)
...
def arraydump(arr):
    if not hasattr(arr, 'keys'):
        return "can not arraydump %s" % type(arr)
    keys = arr.keys()
    keys.sort()
    buf = "{\n"
    for key in keys:
...
def count_bits(bits):
    """
    Takes in a list of ["1","0",1] and returns the number of true values
    """
    count=0
    for bit in bits:
        if bit in [ "1", True, 1]:
...
def string_from_binary(bits):
    """
    bits is [0,1,1,0...]
    We return a string of those bits
    """
    ret=[]
    if len(bits)%8!=0:
        print "Passing the wrong length %d into string_from_binary!"%len(bits)
        

    for i in xrange(len(bits)/8):
        ret+=chr(b(bits[i*8:i*8+8]))
    ret="".join(ret)
    return ret
...
#
#
#        how to handle python integers
#
#
#####################################################
def dInt_n(sint):
    """
    Returns None if dInt is not possible
    """
    try:
        ret=dInt(sint)
...
def dInt(sint):
    ...
    if s[0]=="-":
        negative=True
        s=s[1:] #rip that off for long on python 2.5 (which can't do float("-0x1"))
    if s[0:2]=="0x":
        value=long(s,0)
    else:
...
def backwardsunistring(instr):
    """
    AAA -> A\x00A\x00A\x00 (no null terminator)
    Used by msrpc.py and canvasntlm.py for example
    """
    ret=""
    for c in instr:
        ret+=c+"\x00"
...
def IsInt(astr):
    ...
    Uses dInt_n which is more capatable and capable than int(astr,0)
    """
    if dInt_n(astr):
        return True
    return False
