...
class PElib:
    ...
    def createPEFileBuf(self, shellcode, importante = [ ("advapi32.dll", ["RevertToSelf"])], gui=True ):
...
if __name__ == "__main__":
    ...
    file=""
    try:
        opts, args = getopt.getopt(args, "f:OWEN")
    except:
    ...
    if file:
        if what == OPEN:
            ...
            fd = open(file, "wb")
            fd.write(p.createDotNETPEFileBuf(p.createShellcode(), 0x44444444))
            fd.close()
        else:
            usage(sys.argv[0])
    else:
