...
import os
def procgetparser(procparse,runpass=1, parsetab_name="parsetab"):
    import yacc
    parser=procparse(runpass=runpass)
    #note, never use LALR, always use SLR (sorry)
    ##Rich support for canvas running from non root directory
    #canvas_root_directory=
    yaccer=yacc.yacc(module=parser,debug=0,write_tables=1,method="SLR",tabmodule="MOSDEF%s%s"%(os.path.sep, parsetab_name))
