...
def getClexer():
	...
	debug=0
	myclex=cLex()
	lexer=lex.lex(myclex , debug, optimize=1)
	return lexer  
...
class cLex(object):
	...
	others += ("IF","DO","WHILE","FOR","ELSE","VARIABLENAME","RETURN", "NULL", "STRUCT_MEMBER")
...
class CParse2(object):
	...
	def __init__(self, not_libc=1, imported=None, pointersize=4, remoteresolver=None, vars=None):
		...
		remoteresolver.parse_table_name="MOSDEF%scp2_parsetab_v%s"%(os.path.sep, VERSION)
		#remoteresolver.parse_table_name="cp2_parsetable_v%s"%(VERSION)
		remoteresolver.use_cparse_2=True
	...
	def get_cthing(self, thing_type=None, parents=[], value=None):
		"""
...
'''
	...
	def p_functioncall(self, p):
		...
		###What type of name have we been called with? I mports have cthings
		try:
	...
	def p_importdirective(self, p):  
		...
		#print "---------------------------------#import found, type: %s name: %s dest: %s label: %s"%(importtype, importname, importdest, importlabel)
		...
		##----------------------------------------------------------------------        
			...
			##Only works for Integers, distinguish between ints & floats based types perhaps?
				...
				if op=="<=":
					...
					comp_val=int(p[1].value)+1  ##change this to inc/dec by 1 the last char
					op_val="setifless\n"        ## of the number value ?? work with floats??
				elif op==">=":
					comp_val=int(p[1].value)-1
					op_val="setifgreater\n"
	...
	def p_typedef(self, p):
		...
		if p[1].lower() in ["signed","unsigned"]:
		...
		elif p[2].asterisklength >1 : #Pointers
			...
			devlog("cparse2", "Pointer declared %s -> %s"%(p[0].ctype,p[0].ctype.totype))
...
def getCparser(var_dict):
	"""
    Returns a C parser for MOSDEF and a yaccer
    """
	##TODO - sort this properly, only temporary fudge to aid testing
	target={'os': "Win32", "proc":"X86"}
	#target={'os': "Linux", "proc":"X86"}
	rr=mosdef.getremoteresolver(target['os'], target['proc'])
	#rr_vars=rr.vars
	##TEMP TODO - remove
	#rr_vars["servername"]=None
	#rr_vars["message"]="Pingoo"
	#rr_vars["messagesize"]=len(rr_vars["message"])
	rr_vars=var_dict

	##Initialise our lexer logic and lex, as well as our parser logic
	#parser=CParse2(remoteresolver=rr, vars=rr_vars)
	##Initialise yacc with the parser logic, and specify use of SLR algo rather than standard LR
	#yaccer=yacc.yacc(module=parser,debug=0,write_tables=0,method="SLR")
	
	##Set to use cparse 2 for #import compilation
	rr.use_cparse_2=True
	rr.parse_table_name=VERSION
    
	##Initialise our lexer logic and lex, as well as our parser logic
	parser=CParse2(remoteresolver=rr, vars=rr_vars)
	
	##initialise yacc and write the generated tables to disk
	yaccer=yacc.yacc(module=parser,debug=0,write_tables=1,method="SLR",tabmodule=VERSION)
    
	##Make lex and yacc snowball to each other
...
def testParser():
	...
	data=file(filename,"r").read()
	#print "Testing parser on %s"%filename
	#print "_"*70
	parser,yaccer=getCparser()

	#print "parser & yaccer initialised"


	#print "PARSER VALUE: %s"%(parser.value)
	...
	try:
		...
		parser.tokenizer.vartree.dump_tree()

	#print "raw: %s"%(parser.value)
	#print "IL:\n%s"%''.join(parser.value)
	for blob in parser.value:
		print blob[0]
	return 

if __name__ == "__main__":
	"""
    Test the new cParser
    """
	testLexer()
