...
def run_on_target(target,port,scriptname,protocol="TCP", ipver="IPv4", currentfuzzvariable=0, currentstring=0, sleeptime=0.2, linemode=False, oldstyle=False, threshold=1, maxlength=None, usessl=False):
    """
...
def usage():
    print "USAGE: spike.py -t target -p port -s scriptname -P protocol -i ipver -S sleeptime -V currentvariable[:currentstring] -E "
...
def main(args):
    ...
    #getargs 
    try:
        (opt, args) = getopt.getopt(args, "t:p:s:P:i:V:S:LF:OT:M:E")
    except:
        usage()
        return 
    ...
    if not fileheader:    
        run_on_target(target, port, scriptname, protocol, ipver, currentfuzzvariable, currentstring, sleeptime, linemode, oldstyle, threshold, maxlength, usessl)
    else:
