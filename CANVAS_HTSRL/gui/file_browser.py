    ...
    def run(self):
        ...
        if self.action == "refresh path":
            ...
            if attr["is_dir"] is True:            
                ...
                #print "Refreshing file: %s"%self.path 
                ...
                # Downloading file
                if "downloadpath" in self.args.keys():
                    dpath = args["downloadpath"]
                else:
                    dpath = "Your_Documents/"
                node.vfs_download(self.path, dpath)
            ...
            stat_result = node.vfs_stat(self.path)
            if not stat_result:
                print "Error: File does not exist? %s"%self.path 
                return 
            ...
            file_hider                          = self.engine.getModuleExploit( "hidefile" )
            file_hider.argsDict["filename"]     = self.path
            file_hider.argsDict["passednodes"] += [node]
            
            file_hider.link(self)
            ret = file_hider.run()
...
class browser_window(file_manip):
    ...
    def __init__(self, node, gui, engine, wTree):
        ...
        # check if the node has file hiding capabilities
        ...
        if not self.view:
            print "Some sort of error in the Glade file"
    ...
    def menu_response(self, obj, item):
        ...
        if name == "Download":
            ...
            self.hide_file( self.full_path(fo) )
    ...
    def on_file_treeview_button_press_event(self, obj, event):
        ...
        if event.type == gtk.gdk._2BUTTON_PRESS:
            ...
            for l in self.menulist:
                ...
                mline.connect( "activate", self.menu_response, (l, model.get_value(iter, 0)) )
                mline.show()
                mymenu.append(mline)
    ...
    def hide_file( self, path ):
        ...
        b = browser_actor( self, self.sep )
        b.set_action("hide", {"path":path, "engine":self.engine } )
        b.start()
    ...
    def update_status_bar(self, message):
        ...
        """
        self.status     = message
        self.wTree.get_widget("status_bar").set_text(self.status)
        return 
