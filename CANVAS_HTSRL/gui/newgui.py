...
from NODELOVE import canvasgraph
...
class TargetGui:
    ...
    def __init__(self, targetWidget, engine=None):
        ...
        map_path = 'NODELOVE' + os.sep + 'nasa_map.png'
        try:
...
# new graphed gui layout integration
# this class is used within the old 
# nodegui class to cause epic lulz

class NewNodeGui:
    """ displays graphed interactive MOSDEF nodes """
    
    def __init__(self, CanvasFrame, TargetFrame, engine=None):
        """ init and alter the graph class to specific usage in init """

        self.nodeLines = None
        self.eventbutton = 3
        self.engine = engine
        
        # for target/host network mappings
        if TargetFrame:
            self.targetGraph = TargetGui(TargetFrame, self.engine)
        
        # an actual canvas .. not CANVAS ;)
        self.canvas = CanvasFrame

        # this is the object we use to add vertices and edges
        # it also contains the click handlers and all that
        # see NODELOVE/canvasgraph.py for full details on usage
        self.graph = canvasgraph.NodeCanvas()
        self.graph.set_size_request(2000, 2000)

        # only used for nodegui title display, shouldn't be needed as common code ;)
        self.graph._text = 'CANVAS Graph Node'

        # reconnect the default button press handler
        self.graph.disconnect(self.graph.handlerid)
        self.graph.connect('button-press-event', self.on_graph_line_press)
        
        # for line object compatibility we need a get_menu and a menu_response
        self.graph.__dict__['get_menu'] = self.get_menu
        self.graph.__dict__['menu_response'] = self.menu_response
    ...
    def get_menu(self):
        ...
        # base menu
        menu = ['Knowledge', 'Interfaces']
        if self.nodeLines:
            # reset menu
            menu = []
            # Knowledge then all host knowledge
            menu.append('Knowledge')
            for knowledge in self.nodeLines.hostsknowledge.get_children():
                menu.append(knowledge.text)
                
            # Interfaces then all interfaces
            menu.append('Interfaces')
            for interface in self.nodeLines.interfaces.get_children():
                menu.append(interface.text)
            
        return menu
    
    def add_target_host(self, hostLine):
        """ adds a known host to the target map (and optinally sets the clickhandler) """
        #we only have a targetGraph.graph if they have a modern pycairo
        if self.targetGraph.graph:
            self.targetGraph.graph.add_host(hostLine.interface)
        return
            
    def menu_response(self, obj, item):
        """ the menu response for line object compatibility """
        if self.nodeLines == None:
            raise Exception, 'this node does not contain a valid nodetree'
        
        if item == 'Knowledge':
            # pass through to the actual menu_response we want
            return self.menu_handler(self.nodeLines.hostsknowledge).popup(None, None, None, 0, 0) 
        if item == 'Interfaces':
            return self.menu_handler(self.nodeLines.interfaces).popup(None, None, None, 0, 0)
        # walk the Knowledge list for a handler match
        for host in self.nodeLines.hostsknowledge.get_children():
            if host.text == item:
                return self.menu_handler(host).popup(None, None, None, 0, 0)
        # walk the Interfaces list for a handler match
        for interface in self.nodeLines.interfaces.get_children():
            if interface.text == item:
                return self.menu_handler(interface).popup(None, None, None, 0, 0)
        ...
        for node in self.graph.nodes:
            if x in range(node['x'], node['width'] + node['x']) \
               and y in range(node['y'], node['height'] + node['y']):
                # HANDLE LEFT, MIDDLE, AND RIGHT CLICKS HERE
                pos_x = node['x'] + node['width']
                pos_y = node['y']
                self.eventbutton = event.button
                if event.button == 1:
                    self.nodeLines = node['container']
                    self.menu_handler(obj).popup(None, None, None, 
                                                 event.button, event.time)
                elif event.button == 2:
                    pass
                elif event.button == 3:
                    # on a right click we just pass in the CANVASNode main object
                    # because we don't need specific layout control like we do on
                    # the left click
                    try:
                        # only do it if we're a GRAPH Node right click
                        # we might wanna do something else for a default
                        # right click handler ..
                        if obj._text == self.graph._text:
                            self.nodeLines = node['container']
                            self.menu_handler(self.nodeLines).popup(None, None, None,
                                                                    event.button, event.time)

                    except:
                        # yea yea .. only happens if ._text doesn't exist .. which
                        # we don't care about .. but might occur .. :>
                        pass
                    
        return
    
    def menu_handler(self, obj):
        """ pop up the base menu for knowledge and interfaces """
        
        menuLines = obj.get_menu()
        if menuLines == []:
            raise Exception, 'empty menu !'
        items = {}
        for line in menuLines:
            items[line] = obj.menu_response

        return self.menu(items, menuLines)

    # items in this case is a dict of menu lines and the activate handler for that line
    def menu(self, items, menuLines, title=''):
        """ turn a items handler dict into a menu object we can pop up """
                    
        # we use the List as to not use our original list ordering
        menu = gtk.Menu()
        ...
        def menuCallbackWrapper(w, cb, x):
            # All menu functions migth cause stuff to happen which should force a redraw, and it cant call
            # us back, so we have to wrap it. 
            cb(w, x)
            self.graph.redraw()
        
        for key in menuLines:
            mLine = gtk.MenuItem(key)
            # anything we want to pay attention to we make bold ..
            if key in ['Knowledge', 'Interfaces'] or 'current' in key:
                label = mLine.get_children()[0]
                label.set_markup('<b>%s</b>'% key)
            mLine.connect('activate', menuCallbackWrapper, items[key], key)
            mLine.show()
            menu.append(mLine)
    ...
    def activateTest(self, obj, event):
        print "Triggered Activate placeholder!"
        return
    
class nodegui:
    def __init__(self,nodetree,local,engine, CanvasFrame=None, TargetFrame=None):
        self.engine = engine
        self.local = local
        
        # integrate graphed nodes into engine
        # nodeGraph.graph is the controlling object
        self.nodeGraph = NewNodeGui(CanvasFrame, TargetFrame, engine)
        self.init_app(nodetree, local)
        
    def set_engine(self,engine):
        self.engine=engine

    def addNodeAsVertex(self, node):
        """ adds a CANVAS Node to the graph layout """
    
        title = node.getname()
        label = node.get_interesting_interface() + '\n' + node.nodetype
        
        rowspace = 0
        colspace = 0
        self.nodeGraph.graph.addVertex(title, label, rowspace, colspace, container=node)
        
        # DEBUG PRINTS
        if 0:
            devlog('graph', 'node: ' + repr(node))
            devlog('graph', 'node.parent: ' + repr(node.parent))

            # interfaces
            devlog('graph', 'node.interfaces: ' + repr(node.interfaces))
            for interface in node.interfaces.get_children():
                devlog('graph', 'sub interface: ' + repr(interface))
                # listeners
                for listeners in interface.get_children():
                    devlog('graph', 'sub interface listener: ' + repr(listeners))
        
            # host knowledge (target management correlate to node in target graph)
            devlog('graph', 'node.hostsknowledge: ' + repr(node.hostsknowledge))
            for host in node.hostsknowledge.get_children():
                devlog('graph', 'host: ' + repr(host))
                for c in host.get_children():
                    devlog('graph', 'sub host: ' + repr(c))
        
            # connected nodes
            devlog('graph', 'node.connected_nodes: ' + repr(node.connected_nodes))
            for c_node in node.connected_nodes.get_children():
                devlog('graph', 'sub connected node: ' + repr(c_node))
    ...
    def addNode(self,node):
        ...
        # graph the node .. then go on to do oldschool style
        self.addNodeAsVertex(node)

        # if node has a parent .. add the edge from parent to node vertex
        try:
            if node.parent:
                # node.parent is a nodesList .. the actual node is nodesList.parent :>
                # print repr(node.parent)
                #src = node.parent.parent.get_interesting_interface()
                #dst = node.get_interesting_interface()
                src = node.parent.parent.getname()
                dst = node.getname()
                # print "XXX: adding edge src %s dst %s"% (src, dst)
                self.nodeGraph.graph.addEdge(src, dst)
        except:
            import traceback
            traceback.print_exc(file=sys.stdout)
            raise Exception, 'node.parent list problems'
                
        # set the layout        
        self.nodeGraph.graph.nodes = canvasgraph.NodeLayout().set_layout(self.nodeGraph.graph.nodes, \
                                                edges=self.nodeGraph.graph.edges, \
                                                center_x=0, center_y=0,  \
                                                type='grid')
        
        # redraw the drawing area
        self.nodeGraph.graph.redraw()
    ...
    def addLine(self,lineobj):
        ...
        import hostKnowledge
        # update the target map with any new host lines
        if isinstance(lineobj,  hostKnowledge.hostKnowledge) and self.nodeGraph.targetGraph.graph:
            self.nodeGraph.targetGraph.graph.add_host(lineobj.interface)
            self.nodeGraph.targetGraph.graph.redraw()
    ...
    def delete(self, line):
        ...
        #print "newgui::delete(%s)"%line
        #gc.collect()
        
        # delete node from graph window
        if hasattr(line, 'getname') == True:
            self.nodeGraph.graph.delVertex(line.getname())
        ...
        try:
            pass
            #print "deleting knowlege %s:%s"%(line.tag,line.known)
        except:
            pass
