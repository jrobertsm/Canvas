...
def od2bin(oddata):
    """
    takes od -t x1 data and makes it a binary
    """
    lines=oddata.split("\n")
    data=""
    #print "Line: %s"%lines[0][7:]
    for line in lines:
        data+=binstring(line[7:])
    return data
...
class shelllistener(shellserver):
    ...
    def dospawn(self,command):
        out, rv = self.shellcommand("nohup '%s'" % command)
        if rv != 0:
            raise NodeCommandError("Shell command returned nonzero value %d: %s"  % (rv, out))
        else:
            return out
    ...
    def dounlink(self,filename):
        out, rv = self.shellcommand("rm -f '%s'" % filename)
        if rv != 0:
            raise NodeCommandError("Shell command returned nonzero value %d: %s"  % (rv, out))
        else:
            return out
    ...
    def upload(self,source,dest=".", destfilename=None):
        ...
        command="rm -f %s"%destfile
        out, rv = self.shellcommand(command)
        if rv != 0:
            raise NodeCommandError("Error removing destination file: %s" % out)
        ...
        while i < len(data):
            ...
            for c in tmpdata:
                octeddata+="\\%.3o"%(ord(c))
            command="printf \"%s\" >> %s"%(octeddata,destfile)
            out, rv = self.shellcommand(command)
            if rv != 0:
    ...
    def download(self,source,destdir="."):
        ...
        Downloads a file
        """
        oddata, rv =self.shellcommand("od -v -t x1 %s"%source)
        if rv != 0:
            raise NodeCommandError("Error retrieving od encoded data from %s: %s" % (source, oddata))
        ...
        try:
            ...
            raise NodeCommandError("Unable to write to local temp file %s: %s" % (tmpfile, str(i)))
