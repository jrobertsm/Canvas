...
def setDefaultSocketTimeout(timeout):
    #print "setting default timeout to %s"%timeout
...
# Factory function
#
from socket import AF_INET, SOCK_STREAM, SOL_SOCKET, SO_ERROR, SOCK_DGRAM

def timeoutsocket(family=AF_INET, type=SOCK_STREAM, proto=None):
    if family != AF_INET or type not in [SOCK_STREAM, SOCK_DGRAM]:
        #print "Making normal socket, not timeoutsock"
        if proto:
...
class TimeoutSocket:
    ...
    def connect(self, addr, port=None, dumbhack=None):
        ...
        if not dumbhack:
            ...
            if wearent:
                #special error we get if we got resets to our connect
                #request
                if sock.getsockopt(SOL_SOCKET,SO_ERROR)==10061:
                    raise Error,(111,"Connection refused")
                    
        # If we get here, then we should raise Timeout
        raise Timeout("Attempted connect to %s timed out." % str(addr), errcode )
    # end connect
    ...
    def recv(self, bufsize, flags=0):
        ...
        if self._blocking:
            r,w,e = select.select([sock], [], [], self._timeout)
            if not r:
                raise Timeout("Recv timed out (%s)"%self._timeout)
    ...
    def recvfrom(self, bufsize, flags=0):
        ...
        if self._blocking:
            ...
            if not r:
                raise Timeout("Recv timed out (%s)"%self._timeout)
        return sock.recvfrom(bufsize, flags)
    #end recvfrom
    ...
    def ssl(self, keyfile=None, certfile=None):
        import exploitutils
        return tlssock(self._sock)
        #print "CALLED SSL"
        #return ssl(self._sock, keyfile, certfile)
# end TimeoutSocket
...
def ssl(self, keyfile=None, certfile=None):
    #circular if we put this one level up...
    from exploitutils import tlssock
    connection=tlssock(self._sock)
    return connection
    
if not hasattr(socket, "_no_timeoutsocket"):
    socket._no_timeoutsocket = socket.socket
    oldssl=socket.ssl #save this off
