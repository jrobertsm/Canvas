...
class localNode(CANVASNode):
    ...
    def findInterfaces(self):
        """
        Gets all the interfaces on this machine and ads them to
        our interfaces list
        """
        for ifc in getAllLocalIPs():
            devlog("localNode", "Found ip: %s"%ifc)
            #ifc is a ['vmnet1', '192.168.159.1', 4294967040L] list
    ...
    def findLocalHosts(self):
        ...
        for ip in getIPfromARPTable() + CanvasConfig['local_static_ip_list', ""].split():
            argsDict["host"]=ip
            self.log("Adding host %s in localNode" % ip)
            app.run()
        return 1
    ...
    def recv(self,sock,length):
        ...
        datasets
        """
        devlog('localNode::recv',"Node %s recving %d bytes" % (self.getname(), length))
        data=""
        datalist=[]
        readlength=0
        while readlength<length:
            ...
            try:
                ...
                devlog('localNode::recv', "Socket.error recieved...nonblocking not on?")
                continue
            #devlog('localNode::recv', "recved %d bytes: %s" % (len(tmp), hexprint(tmp)))
            if tmp=="":
                self.log("localnode: Connection broken?!?")
                break
            readlength+=len(tmp)
    ...
    def loadSavedHosts(self):
        self.hostsknowledge.load_all_hosts(prefix=self.getname()+"_")
