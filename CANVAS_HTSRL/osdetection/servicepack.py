    ...
    def getSPInfo(self):
        if self.baseos.count('XP') > 0:
            ...
            if len(connectionList)==0: #anonymously we didn't get anything back, so SP2/SP3 it is
                result=self.doIfids()
                ##Rich sometimes none is returned from doIfids so we need to check for that before unpacking
                if not result:
                    return
                for (port,endpoint,uuid,version) in result:
                    if uuid=='d4781cd6-e5d3-44df-ad94-930efe48a887' or uuid=='9556dc99-828c-11cf-a37e-00aa003240c7':
            ...
            connectionList = self.getEndpoints('4b112204-0e19-11d3-b42b-0000f81feb9f')
            found = False
            for endpoint in connectionList:
