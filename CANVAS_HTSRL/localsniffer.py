...
class localsniffer(Thread):
    def __init__(self, prelooping_delay=1, engine=None, driver=None, dstats=True):
    ...
    def realrun(self):
        ...
        try:
            parser = packetParser(self.driver)
        except:
        ...
        while 1:
            ...
            for each in self.callbacks:
                ...
                if each[0].match(parser):
                    ...
                    #thread doesn't die!
                    devlog("localsniffer", "MATCHED on packet for %s\n"%parser.fields)
                    try:
                        #call our callback
                        each[1](parser)
                    except:
