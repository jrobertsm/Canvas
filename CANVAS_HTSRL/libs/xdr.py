...
class Packer:
        ...
        def pack_uint(self, x):
                #print "X=%s"%x
                self.buf = self.buf + (chr(int(x>>24 & 0xff)) + chr(int(x>>16 & 0xff)) + \
        ...
        def pack_uhyper(self, x):
                self.pack_uint(int(x>>32 & 0xffffffffL))
                self.pack_uint(int(x & 0xffffffffL))
        ...
        def pack_stringarray(self,stringarray):
                n=len(stringarray)
                self.pack_uint(n)
...
class Unpacker:
        ...
        def done(self):
                if self.pos < len(self.buf):
                        raise RuntimeError, 'Unpacker :: unextracted data remains'

        def unpack_uint(self):
                #print "Len(buf)=%d"%len(self.buf)
                i = self.pos
                self.pos = j = i+4
                data = self.buf[i:j]
                if len(data) < 4:
