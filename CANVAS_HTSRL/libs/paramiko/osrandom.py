...
try:
    from libs.Crypto.Util import winrandom
except ImportError:
    winrandom = None
...
try:
    from libs.Crypto.Util.randpool import RandomPool
except ImportError:
    RandomPool = None
...
import random
class InsecureRandomPool (BaseOSRandomPool):
    def __init__(Self):
        pass

    def get_bytes(self, n):
        ret=[chr(random.randint(0,256)) for x in xrange(0,n)]
        return "".join(ret)
...
# CANVAS does not give up, we just use an insecure one when we can't get a real one
if osrandom_source is None:
    osrandom_source="Insecure Rand"
    DefaultRandomPoolClass = InsecureRandomPool
