...
class TLSRecordLayer:
    ...
    def close(self):
        ...
        Modified by dave to break after 5 tries - we don't want
        this blocking forever the way it does against IIS's SSL 
        """
        self.sock.set_timeout(0.1)
        if not self.closed:
            i=0
            
            results=self._decrefAsync()
    ...
    def _calcPendingStates(self, clientRandom, serverRandom, implementations):
        if self.session.cipherSuite in CipherSuite.aes128Suites:
            ...
            raise AssertionError()
        digestmod = sha
        if self.session.cipherSuite in CipherSuite.md5Suites:
            macLength = 16
            digestmod = md5
