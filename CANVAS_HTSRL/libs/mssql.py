...
class Packet:
    ...
    def raw(self):
        buf= self._raw()
        self.LastPacket = LAST_PACKAGE # we leave it like this, for now
...
class Response(Packet):
 ...
 """
    ...
    def __init__(self):
        ...
        self.parse_tokens= {0x21:self.parse_pass, 0x71:self.parse_pass, 0x79:self.parse_pass, 0x7c:self.parse_pass,\
    ...
    def getTokenName(self, token_num):
        try:
            return self.tokens_names(token_num)
        except KeyError:
            return ""
    ...
    def get(self, data):
        ...
        while 1:
            ...
            except KeyError: # Unsupported token :(
                devlog("mssql","Unsupported token %s"%token)
                break 

            if token == 0xFD or token == 0xFE or token == 0xFF:
...
class Query(Packet):
    ...
    def setQuery(self, query):
        self.query=query
    def _raw(self):
        return string2Unicode(self.query)
