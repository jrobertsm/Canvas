...
class header:
    ...
    def getStringArguments(self):
        """
        Gets our url arguments as a string.
        """
        request=""
        ...
        if self.useRawArguments:
            ...
            if len(self.URLargsDict) > 0:
                request+="?"
                request+=daveutil.joinargs(self.URLargsDict,orderlist=self.orderlist)
        return request
    ...
    def processProxyUrl(self, proxyurl):
        ...
        if not self.clientisSSL:
            ...
            if len(self.connectHost.split(":"))>1:
                ...
                self.connectPort=dInt(self.connectHost.split(":")[1])
...
class body:
    ...
    def readBlock(self,connection,size):
        ...
        while targetsize > gotlength:
            ...
            #print "Targetsize=%d, gotlength=%d" % (targetsize,gotlength)
            ...
            if readtillclosed and size!=0:
                ...
                while 1:
                    ...
                    #    break #no ssl for now
                        ...
                        print "spkproxy: An exception occured: %s" % e
                        connection.close()
                        break
...
#####FIXME
    ...
    def connectToWebServer(self, myheader):
        ...
        if self.clientisSSL or myheader.clientisSSL or myheader.connectPort==443:
            ...
            if not self.haveSocket:
                ...
                devlog("spkproxy","Doing TLS Connection")
                try:
    ...
    def sendRequest(self,myheader,mybody, noresponse=False):
        """
        ...
        while returncode=="100":
            ...
            while serverheader.isdone()==0:
                try:
                    ...
                    devlog("spkproxy","Timeout exception: continuing")
                    continue
                except socket.error:
...
class filetype_str:
    """
    initialize this with a string and it will pretend to be a file
    supports readline, read, to make people who use urlopen
    happy
    """
    def __init__(self,astr):
        self.data=astr
        self.index=0
        #print "Initialized with %s"%astr
        return
    ...
    def readline(self):
        ...
        if index==-1:
            #we did not find a newline, so return the rest of the data
            self.index=len(self.data) #end this buffer
            return data
        data=data[:index]
        self.index=self.index+index
        #print "Readline returning %s"%data
        return data
    
    def read(self,size=None):
        #print "Read size=%d index=%d"%(size,self.index)
        if size==None:
            data=self.data[self.index:]
        else:
            data=self.data[self.index:self.index+size]
        self.index+=len(data)
        #print "Read returning: %s"%data
        return data
...
class UserAgent(object):
    ...
    """
    def __init__(self, basepath, exploit=None, auth=None, hostname=None, proxyhost="", proxyport=None):
    ...
    def GET(self, args, noresponse=False, entireresponse=False):
        extraheaders=self.getExtraHeaders()
        fd=urlopen(self.basepath+args, extraheaders=extraheaders, exploit=self.exploit, hostname=self.hostname, auth=self.auth, useragent=self, proxyhost=self.proxyhost, proxyport=self.proxyport,noresponse=noresponse, entireresponse=entireresponse)
        if noresponse:
    ...
    def PUT(self, args, data, noresponse=False):
        extraheaders=self.getExtraHeaders()
        fd=urlopen(self.basepath+args, extraheaders=extraheaders, data=data, verb="PUT", exploit=self.exploit, hostname=self.hostname, auth=self.auth, useragent=self, proxyhost=self.proxyhost, proxyport=self.proxyport, noresponse=noresponse)
        if noresponse:
            #we return the file descriptor and don't wait for a response
            return fd 
        else:
    ...
    def OPTIONS(self, entireresponse=True, noresponse=False):
        extraheaders=self.getExtraHeaders()
        fd=urlopen(self.basepath, extraheaders=extraheaders, verb="OPTIONS", exploit=self.exploit, hostname=self.hostname, auth=self.auth, useragent=self, proxyhost=self.proxyhost, proxyport=self.proxyport, noresponse=noresponse)
        if noresponse:
            #we return the file descriptor and don't wait for a response
            return fd 
        else:
    ...
    def POST(self, args, data, extraheaders=None, noresponse=False):
        ...
        fd=urlopen(self.basepath+args, data=data, verb="POST", exploit=self.exploit, hostname=self.hostname, auth=self.auth, extraheaders=extraheaders, useragent=self,  proxyhost=self.proxyhost, proxyport=self.proxyport, noresponse=noresponse)
        if noresponse:
            #we return the file descriptor and don't wait for a response
            return fd 
        else:
...
def urlopen(url,extraheaders=None, data=None,proxies=None, exploit=None, verb="GET", auth=None, hostname=None, useragent=None, proxyhost="", proxyport=None, entireresponse=False, noresponse=False):
    """
    Replacement for urllib.urlopen that uses SPKProxy:
    Benefits:
    o Does TLSlite properly (urllib is not compatable with our timeoutsocket.py
    o MOSDEF (and CANVAS Node) compatable
    ...
    myconn = spkProxyConnection(None, None, ntlm=ntlmtuple)
    ...
    response = myconn.sendRequest(myheader, mybody, noresponse=noresponse)
    if noresponse:
        return None 
...
class sForm(object):
    ...
    def __str__(self):
        return self.action+"?"+self.getDefaultBody()
