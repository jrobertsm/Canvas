...
gateway_from_iface={}
try:
    data=file("/proc/net/route").readlines()
    have_routes=True 
except:
    have_routes=False
if have_routes:
...
mac_from_ip={}
try:
    data=file("/proc/net/arp").readlines()
    have_arp=True 
except:
    have_arp=False
if have_arp:
...
def get_ethernet_header(target):
    """
        ...
        try:
            ...
            for answer in range(answer_rrs):
                ...
                if datalength==4:
                    #address
                    addr_data=packet[:4]
                    attribs["answer_address"]=socket.inet_ntoa(addr_data)
...
class packetParser:
    ...
    def setPacket(self,packet):
        ...
        if self.isEth():
            ...
            # This is useful if the callback needs access to all of the data
            ...
            devlog("sniffer", "SMAC: "+ self.prettymac(frommac) + " DMAC: "+ self.prettymac(tomac) + " ETHTYPE: "+ str(type))
            ...
            if type == ETHTYPE['ARP']:
                ...
                if t:
                    ...
                    devlog("sniffer", "type: ARP SRCIP: "+ srcip + " DSTIP: "+ dstip + " ARPTYPE: "+ str(arptype))
                ...
                devlog("sniffer", "type: IPV4 SRCIP: "+ source +" DSTIP: "+ dest + " IPPROTO: "+ str(protocol) + " TTL: "+ str(ttl))
                ipsource = source
                ipdest = dest

                if protocol=="TCP":
                    (offset,source,dest,flags,chksum)=self.TCP(offset=offset)
                    ...
                    devlog("sniffer", "type: TCP SRCPORT: "+ str(source) + " DSTPORT: "+ str(dest))
                ...
                if protocol == "UDP":
                    ...
                    devlog("sniffer", "type: UDP SRCPORT: "+ str(source) + " DSTPORT: "+ str(dest))
                    if self.is_nbHostAnnounce():
                        devlog("sniffer", " HOST ANNOUNCEMENT")
                        upint = self.get_nbUpdatePeriod()
                        if self.nbMatchIntervals(upint):
                            nbname = self.get_nbName()
                            nbcomment = self.get_nbComment()
                            devlog("sniffer", " Name: "+ nbname +" Comment: "+nbcomment)
                        ...
                        dns_reply_parse(self.packet, self.attribs)
                ...
                if protocol == "ICMP":
                    ...
                    devlog("sniffer", "type: ICMP CODE: "+ str(code))
            ...
            try:
                ...
                for st in SUBTYPE_80211:
                    if SUBTYPE_80211[st] == subtype:
                        ...
                        devlog("sniffer", "Subtype: "+ str(hex(subtype))+" Encrypted: "+ str(encr) + " Size: "+ str(len(self.packet)))

                        (smac, bssid, dmac) = self.get_80211_addrs(st)                  
    ...
    def get_80211_type(self):
        if(len(self.packet)>(1+self.offset)):
    ...
    def check_80211_enc(self):
        if(len(self.packet)>(27+self.offset)):
    ...
    def get_80211_auth_challenge(self, seq):
        ...
        if (seq == 2) and (len(self.packet)>=rd_size):
    ...
    def get_80211_auth_challenge_payload_broken(self):
        start_offset = 0x18 #everything from the start of the wireless management frame
        if len(self.packet)>start_offset:
    ...
    def get_80211_addrs(self, st):
        ...
        if self.has_80211_srcifo(st):
            ...
            # Data, null func packets make our life harder and have the fields opposite
            if st == "DATA" or st == "NULL_FUNC":
                bssid, smac = smac, bssid
        else:
            smac = bssid = False
                           
        dmac = self.get_80211_dst_mac()
                        
        # QOS DATA packets got destination swapped with bssid
        if st == "QOSDATA":
    ...
    def get_80211_mac(self, offset):
        ...
        devlog("sniffer", "We failed for packet: "+repr(self.packet)+ " offset was: "+ str(offset) +"length of packet was: "+ str(len(self.packet)))
    ...
    def get_80211_status_code(self, st):
        ...
        if check and (len(self.packet) >= upper):
    ...
    def get_80211_tagged_parameters(self):
        if len(self.packet) >= (self.offset+0x1c):
    ...
    def LAYERI80211_BECON(self, st):
        ...
        if(len(self.packet)>=(38+curoffset)):
    ...
    def is_nbHostAnnounce(self):
        if len(self.packet)>0xd2 and ord(self.packet[0xd2])==1:
            return True
    ...
    def IPv4(self):
        ...
        devlog("sniffer","Adding sniffed IP %s"%source)
    ...
    def prettyprint(self):
        ...
        if protocol=="TCP":
            (offset,source,dest,flags,checksum)=self.TCP()
            print "TCP: SourcePort(%d) DestPort(%d)"%(source,dest)
            for key in TCPFLAGS:
                if ord(flags) & key:
                    print TCPFLAGS[key],
    ...
    def ARP(self):
        ...
        if opcode == ARP_REQUEST or opcode == ARP_REPLY:
            ...
            devlog("sniffer", "Adding sniffed ip: %s"%senderIP)
...
class sender:
    ...
    def __init__(self, iface=None, target=None):
        """
        This is called with iface=None and target=None from
        self.can_rawsock()
        """
        if iface==None and target!=None:
            ...
            iface=get_iface_from_target(target)
        if not iface:
            devlog("sniffer", "Using default interface because we could not find one for %s"%target)
            iface="eth0" #default (linux only!)
